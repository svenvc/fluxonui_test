defmodule FluxonUITestWeb.UserAuthTest do
  use FluxonUITestWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias FluxonUITest.Accounts
  alias FluxonUITest.Accounts.Scope
  alias FluxonUITestWeb.UserAuth

  import FluxonUITest.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, FluxonUITestWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      FluxonUITestWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        FluxonUITestWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
